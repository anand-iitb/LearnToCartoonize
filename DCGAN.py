...
netG.load_state_dict(torch.load(
    'weights/netG_epoch_99.pth', map_location=device))
print(netG)
...
netD.load_state_dict(torch.load(
    'weights/netD_epoch_99.pth', map_location=device))
print(netD)
...
for epoch in range(niter):
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            vutils.save_image(
                real_cpu, 'output/real_samples.png', normalize=True)
            fake = netG(fixed_noise)
            vutils.save_image(
                fake.detach(), 'output/fake_samples_epoch_%03d.png' % (epoch), normalize=True)
